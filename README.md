...
Use **Edit the system environment variables** â†’ **Environment Variables** â†’ **User variables**. Do not put the key in source code, `config.json`, the repository, or a command committed to shell history.
...
- Maximum recording length: 5â€“180 seconds.
...
Run `python gpt_transcribe.py --list-devices`, select an input device in Settings, and confirm microphone access is enabled in Windows Privacy & security â†’ Microphone.
