...
class App:
    ...
    def run(self) -> None:
        ...
        self.icon = pystray.Icon(
            ...
            menu=pystray.Menu(
                ...
                pystray.MenuItem("Settingsâ€¦", self._menu_settings),
    ...
    def start_recording(self) -> None:
        ...
        with self.state_lock:
            ...
            self.status = "Listeningâ€¦ press the hotkey to finish"
            self.icon.update_menu()
            self.icon.icon = create_icon_image(recording=True)
            self.icon.title = f"{APP_NAME} â€” Listening"
        self._beep(880)
    ...
    def stop_recording(self) -> None:
        with self.state_lock:
            ...
            self.status = "Transcribingâ€¦"
            self.icon.update_menu()
            self.icon.icon = create_icon_image(recording=False)
            self.icon.title = f"{APP_NAME} â€” Transcribing"
        try:
            if stream:
                stream.stop()
                stream.close()
    ...
    def _finish_with_status(self, status: str, notify: bool) -> None:
        with self.state_lock:
            ...
            if self.icon:
                self.icon.update_menu()
                self.icon.title = f"{APP_NAME} â€” {status}"
        if notify:
